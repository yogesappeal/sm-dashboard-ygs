  ...
  return (
    ...
    <div className="flex flex-col h-full overflow-hidden relative -m-4 md:-m-6">
        ...
        <div className="flex-1 min-w-0">
          {isLoading ? (
            ...
            <h1 className="text-lg font-semibold text-slate-800 truncate">
              {po?.po_number ?? 'Purchase Order Detail'}
            </h1>
          )}
        </div>
        {po && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {po.status === 'PO Draft' && (
              <PermissionGuard action="po:edit">
                <Link
                  href={`/purchase-orders/supplier/new?edit=${id}`}
            ...
            {/* Send email — on Submitted */}
            {po.status === 'PO Submitted' && (
              <button
                onClick={() => setSendEmailDialog(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-white bg-primary hover:bg-primary-dark rounded-lg transition-colors"
              >
                <Send size={14} />
                Send PO
              </button>
            )}
            {/* Accept / Reject — on Submitted */}
            {po.status === 'PO Submitted' && (
              <>
                <button
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to accept this PO? This will mark it as PO Sent."
        confirmLabel="Accept"
        variant="default"
      ...
      <ConfirmDialog
        ...
        title="Send PO Email"
        description="This will automatically send the PO email to the supplier. Continue?"
