    ...
    <div ref={wrapperRef} className="relative w-full">
      ...
      <div
        className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-all duration-200 ${
          focused
            ? 'border-primary/50 bg-white shadow-sm shadow-primary/10'
      ...
      >
        {loading ? (
          ...
          <Search size={15} className={`shrink-0 transition-colors duration-200 ${focused ? 'text-primary' : 'text-slate-400'}`} />
        )}
        <input
          ref={inputRef}
          type="text"
              ...
              <li key={item.id}>
                <button
                  ...
                  className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors duration-100 ${
                    activeIndex === idx ? 'bg-primary/5' : 'hover:bg-slate-50'
                  }`}
                >
                  {/* Icon */}
                  <div className={`mt-0.5 p-1.5 rounded-lg shrink-0 transition-colors ${activeIndex === idx ? 'bg-primary/10' : 'bg-slate-100'}`}>
                    <FileText size={13} className={activeIndex === idx ? 'text-primary' : 'text-slate-500'} />
                  </div>
